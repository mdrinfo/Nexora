...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            <h2 class="text-xl font-semibold text-gray-800 mb-6">Modifier la catégorie: {{ $category->name }}</h2>
            ...
            <form action="{{ url('/admin/categories/' . $category->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="destination" id="destination" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2 text-gray-900">
                        <option value="kitchen" {{ old('destination', $category->destination) == 'kitchen' ? 'selected' : '' }}>Cuisine 🍳</option>
                        <option value="bar" {{ old('destination', $category->destination) == 'bar' ? 'selected' : '' }}>Bar 🍸</option>
                    </select>
                ...
                <div class="mb-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="is_drink" id="is_drink" value="1" {{ old('is_drink', $category->is_drink) ? 'checked' : '' }}
                ...
                <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                    <button type="button" onclick="if(confirm('Supprimer cette catégorie ?')) document.getElementById('delete-form').submit();" class="text-red-600 hover:text-red-800 text-sm font-medium">
                        Supprimer
                    </button>
            ...
            <form id="delete-form" action="{{ url('/admin/categories/' . $category->id) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
